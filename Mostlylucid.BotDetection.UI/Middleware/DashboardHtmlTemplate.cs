...
<head>
    ...
    <script defer src=""https://cdn.jsdelivr.net/npm/alpinejs@3.13.5/dist/cdn.min.js"" nonce=""{cspNonce}""></script>
...
<div x-data=""dashboard()"" x-init=""init()"" class=""min-h-screen"">
    ...
    <header class=""brand-header sticky top-0 z-50"">
        <div class=""dashboard-shell px-4 py-2 flex items-center justify-between"">
            <div class=""flex items-center gap-3"">
                <span class=""brand-wordmark text-lg"" style=""color: var(--sb-accent)"">
                    <span style=""font-style: italic"">stylo</span><span style=""color: var(--sb-brand-strong)"">bot</span>
                </span>
            ...
            <div class=""flex items-center gap-2"">
                ...
                <a href=""/"" class=""btn btn-xs btn-ghost btn-circle"" title=""Home"">
    ...
    <!-- Your detection data (inline JSON from same request) -->
                        ...
                        <template x-if=""activeDetail"">
                            <div class=""flex items-center gap-1"">
                                ...
                                <span class=""text-sm font-bold uppercase tracking-wider hero-accent-text""
                                      x-text=""(activeDetail.botName || activeDetail.sig.substring(0, 8))""></span>
                            ...
                            <div class=""flex items-start gap-3 mb-3"">
                                ...
                                <div class=""flex-1 min-w-0"">
                                    <h3 class=""text-lg font-bold"" :class=""activeDetail?.isBot ? 'text-error' : 'text-success'""
                                        x-text=""activeDetail?.botName || (activeDetail?.isBot ? 'Bot Detected' : 'Human Visitor')""></h3>
                        ...
                        <div class=""card-body p-4"">
                            ...
                            <div x-show=""probabilityHistory.length > 1"" class=""bg-base-300/30 rounded-lg p-2 mb-3"">
                                ...
                                <svg :width=""sparklineWidth + 8"" height=""48"" class=""overflow-visible"">
                                    ...
                                    <!-- Current value dot -->
                                    <circle :cx=""(probabilityHistory.length - 1) * 4"" :cy=""(1 - probabilityHistory[probabilityHistory.length - 1]) * 40"" r=""3"" :fill=""sparklineColor"" />
                                    <!-- Labels -->
                            ...
                            <div class=""px-3 py-2.5 transition-colors duration-300 cursor-pointer""
                                ...
                                <div class=""flex items-center gap-2 mb-1"">
                                    ...
                                    <span class=""text-xs font-semibold flex-1 min-w-0 truncate""
                                          x-text=""v.botName || v.botType || (v.isBot ? 'Unknown Bot' : 'Visitor')""></span>
...
function dashboard() {{
    return {{
        ...
        // --- Data Fetching ---
        async fetchSummary() {{
            try {{ var r = await fetch('{options.BasePath}/api/summary'); if (r.ok) this.summary = toCamel(await r.json()); }} catch(e) {{}}
            this.summaryLoading = false;
        }},

        async fetchSignatures() {{
            try {{
                var r = await fetch('{options.BasePath}/api/signatures?limit=50');
                if (r.ok) {{
                    var sigs = toCamel(await r.json()) || [];
                    var self = this;
        ...
        async fetchDetections() {{
            try {{
                var r = await fetch('{options.BasePath}/api/detections?limit=200');
                if (r.ok) {{
                    var items = (toCamel(await r.json()) || []).reverse();
                    var self = this;
                    items.forEach(function(d) {{ self.trackEndpoint(d); self.upsertVisitor(d); }});
                }}
            }} catch(e) {{}}
        }},
        ...
        async fetchCountries() {{
            try {{
                var r = await fetch('{options.BasePath}/api/countries');
                if (r.ok) this.countries = toCamel(await r.json()) || [];
            }} catch(e) {{}}
        ...
        async fetchClusters() {{
            try {{
                var r = await fetch('{options.BasePath}/api/clusters');
                if (r.ok) this.clusters = toCamel(await r.json()) || [];
            }} catch(e) {{}}
        ...
        fetchYourDetection() {{
            try {{
                var el = document.getElementById('your-detection-data');
                if (el) {{
            ...
            // No embedded detection yet — hide loading, wait for SignalR to deliver it
