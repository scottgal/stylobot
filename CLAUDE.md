...
- `Orchestration/SqliteResponseHistoryStore.cs` (8.11.0+) - write-behind-LFU durable tier for `ResponseCoordinator`'s per-client history (404-scan counts, honeypot hits, auth failures) that drives `ResponseBehaviorAtom` scoring; `response_client_history` table, sibling db file next to `DatabasePath`. Optional in DI - a host that doesn't register it degrades to in-memory-only tracking rather than failing to construct.
...
Legacy `/stylobot/countries`, `/stylobot/clusters` (Leiden community detection), and UA-family views remain reachable for back-compat. `BotType.Internal` (LAN traffic) is classified, listed, and filterable but never throttled. As of 8.11.0, internal traffic is **excluded from every read path by default** (Traffic/Site/Visitors, SQLite + Postgres, direct + composed) - a top-level toggle (`?internal=show`) opts back in per the same audience-filter resolution the envelope cache key uses; the exclusion keys on both `bot_type='Internal'` and known internal `bot_name`s (self-hosted monitors) since `bot_type` is often empty on those rows. A host may also register the optional `IDashboardPathSuppressor` DI seam to hide specific endpoint paths (and their aggregate counts) from the dashboard display only - detection and counting are untouched. `StyloBotDashboardOptions.SuppressVendorScripts` lets a host layout that already ships its own htmx/SignalR bundle stop the `<sb-live-updates>` tag helper from emitting a duplicate copy.
