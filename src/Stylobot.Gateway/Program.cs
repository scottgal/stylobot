...
try
{
    ...
    //                                          publicly reachable)
    // PROXY protocol: recover the real client IP when stylobot sits behind a
    // TCP proxy / SNI router / tunnel (which can't add X-Forwarded-For). When
    // enabled this binds endpoints in code so the PP parser runs before TLS;
    // when disabled it's a no-op. Shared with the commercial GatewayHost.
    builder.AddProxyProtocolGatewayEndpoints();
...
// (PROXY-protocol CIDR parsing now lives in ProxyProtocolKestrelExtensions,
//  shared by the FOSS gateway and the commercial GatewayHost.)