...
///     <see cref="CommonUserAgentService"/>. Was a
///     <see cref="Microsoft.Extensions.Hosting.BackgroundService"/> with a
///     configurable <c>Task.Delay</c> loop scraping useragents.me; now
///     subscribes to <see cref="TickCadence.Tick1h"/>. Same shape as
///     <see cref="BrowserVersionService"/>: gates the remote fetch on
///     "last-success older than configured interval".
/// </summary>
public sealed class CommonUserAgentServiceTests
{
    private static IOptions<BotDetectionOptions> Options(
        bool versionAgeEnabled = true,
        bool remoteFeedEnabled = true)
    {
        var o = new BotDetectionOptions();
        o.VersionAge.Enabled = versionAgeEnabled;
        o.VersionAge.UpdateIntervalHours = 24;
        o.DataSources.BrowserVersions.Enabled = remoteFeedEnabled;
    ...
    public void Constructor_subscribes_to_Tick1h_with_service_name()
    {
        ...
        using var sut = new CommonUserAgentService(
            NullLogger<CommonUserAgentService>.Instance,
            Options(),
        ...
        sub.Name.Should().Be("CommonUserAgentService");
    ...
    [Fact]
    public async Task OnTickAsync_is_noop_when_remote_feeds_disabled()
    {
        var coordinator = new RecordingScheduleCoordinator();

        using var sut = new CommonUserAgentService(
            NullLogger<CommonUserAgentService>.Instance,
            Options(remoteFeedEnabled: false),
            new NullHttpClientFactory(),
            coordinator);
    ...
    public void Dispose_unsubscribes_from_coordinator()
    {
        ...
        var sut = new CommonUserAgentService(
            NullLogger<CommonUserAgentService>.Instance,
            Options(),
            new NullHttpClientFactory(),
            coordinator);
