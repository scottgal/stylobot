...
public sealed class SqliteSessionStoreSinceFilterTests : IAsyncLifetime
{
    ...
    public async Task Returns_all_sessions_when_since_is_null()
    {
        ...
        await _store.AddSessionAsync(MakeSession("sig-a", now.AddDays(-10)));
        await _store.AddSessionAsync(MakeSession("sig-b", now.AddDays(-1)));
        await _store.AddSessionAsync(MakeSession("sig-c", now));
    ...
    public async Task Excludes_sessions_that_ended_before_since_cutoff()
    {
        ...
        await _store.AddSessionAsync(MakeSession("old", now.AddDays(-8)));    // before cutoff
        await _store.AddSessionAsync(MakeSession("recent", now.AddDays(-1))); // after cutoff
    ...
    {
        var now = DateTime.UtcNow;
        await _store.AddSessionAsync(MakeSession("old-a", now.AddDays(-10)));
        await _store.AddSessionAsync(MakeSession("old-b", now.AddDays(-15)));
    ...
    public async Task Includes_session_at_exact_since_boundary()
    {
        ...
        await _store.AddSessionAsync(MakeSession("boundary", cutoff));
    ...
    public async Task Combines_since_with_isBot_filter()
    {
        ...
        await _store.AddSessionAsync(MakeSession("bot-old",    now.AddDays(-10), isBot: true));
        await _store.AddSessionAsync(MakeSession("bot-recent", now.AddDays(-1),  isBot: true));
        await _store.AddSessionAsync(MakeSession("human-recent", now,            isBot: false));
    ...
    public async Task IsBot_false_filter_with_since_excludes_old_human_sessions()
    {
        ...
        await _store.AddSessionAsync(MakeSession("human-old",    now.AddDays(-5), isBot: false));
        await _store.AddSessionAsync(MakeSession("human-recent", now.AddDays(-1), isBot: false));
        await _store.AddSessionAsync(MakeSession("bot-recent",   now,             isBot: true));
    ...
    {
        var now = DateTime.UtcNow;
        await _store.AddSessionAsync(MakeSession("oldest", now.AddHours(-3)));
        await _store.AddSessionAsync(MakeSession("newest", now));
        await _store.AddSessionAsync(MakeSession("middle", now.AddHours(-1)));
    ...
    public async Task Limit_is_respected_after_applying_since_filter()
    {
        var now = DateTime.UtcNow;
        for (var i = 0; i < 5; i++)
            await _store.AddSessionAsync(MakeSession($"sig-{i}", now.AddMinutes(-i)));
