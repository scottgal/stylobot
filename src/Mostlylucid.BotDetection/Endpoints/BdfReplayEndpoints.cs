...
public static class BdfReplayEndpoints
{
    ...
    {
        // scopeFactory is injected (rather than resolved from httpContext) so this
        // handler keeps a 2-parameter shape: a single-HttpContext handler would be
        // convertible to RequestDelegate and bind MapPost's terminal overload
        // (IEndpointConventionBuilder), breaking the .Accepts/.Produces chain.
        //
        // Each replayed request runs in its own DI scope (below) so it gets a fresh
        // per-request SignalSink, exactly like a real HTTP request. BotDetectionOrchestrator
        // is scoped; the ephemeral orchestrator has no verdict/signature cache (every
        // DetectAsync runs the full engine), so a scenario's verdict matures across
        // requests via the shared singleton stores without any cache-disable policy.
        ...
        for (var i = 0; i < requests.Count; i++)
        {
            ...
            // Fresh DI scope per replayed request -> fresh per-request SignalSink,
            // exactly like a real HTTP request. Shared singleton stores (fingerprint,
            // behavioral waveform) still accumulate across the scenario so the verdict
            // matures. Disposed at the end of the iteration.
            ...
            // Run detection through the ephemeral BotDetectionOrchestrator resolved from
            // the per-request scope (the same concrete type the middleware uses). The
            // ephemeral path has no verdict/signature cache, so every request runs the
            // full engine and the scenario verdict matures naturally.
            // Point IHttpContextAccessor at the synthetic context. The ephemeral atoms read
            // the request via _httpContextAccessor.HttpContext (AsyncLocal), NOT a passed
            // parameter — so UserAgentAtom (UA), TlsFingerprintAtom (forwarded X-JA3/JA4), and
            // the TransportHeaderTrust gate (SyntheticTrustOverrideKey in .Items) would otherwise
            // see the ambient /replay POST context instead of this scenario's request. Detection
            // reads the sink (hydrated from syntheticContext) so verdicts were already correct,
            // but the accessor-reading atoms silently classified against the wrong context.
            var httpContextAccessor = requestScope.ServiceProvider.GetService<IHttpContextAccessor>();
            if (httpContextAccessor is not null)
                httpContextAccessor.HttpContext = syntheticContext;

            var orchestrator = requestScope.ServiceProvider
                .GetRequiredService<Orchestration.Atoms.BotDetectionOrchestrator>();
            Orchestration.AggregatedEvidence evidence;
            try
            {
                evidence = await orchestrator.DetectAsync(
                    syntheticContext, httpContext.RequestAborted);
            }
            catch (Exception ex)
            {
