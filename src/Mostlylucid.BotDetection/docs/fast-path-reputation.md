...
The detector runs first in Wave 0 (priority 3) with no dependencies. It queries the `IPatternReputationCache` using the raw User-Agent and client IP to find previously classified patterns. This is a circuit-breaker style check that avoids running the full 32-detector pipeline, including `Threat Intelligence`, for requests that have already been conclusively identified.
