...
///     Signatures (Web Bot Auth) and generic signed bearer tokens — behind one
///     abstraction. Both are <c>sig + claims + expiry</c> shapes; one interface
///     over both prevents parallel crypto pipelines. FOSS carries no knowledge of
///     what a signed bearer token <i>means</i> (licensing, entitlements, etc.) —
///     it verifies the signature and surfaces the raw claims; the caller
///     interprets them.
///     <para>
...
public readonly record struct TokenInput(
    ...
    string RawValue,              // full signature-input+signature block for 9421; base64 blob for a signed bearer token
...
public sealed record TokenVerdict(
    ...
    string? SubjectName,          // AgentName for 9421 (from PublicKeyRegistry); the configured subject claim for a signed bearer token
    IReadOnlyDictionary<string, string>? Claims,   // signed-token claims OR the 9421 signature parameters
