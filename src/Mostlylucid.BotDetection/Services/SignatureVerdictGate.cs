...
public sealed class SignatureVerdictGate
{
    ...
    private readonly InProcessVerdictCache? _l0Cache;
    ...
    public SignatureVerdictGate(
        ...
        InProcessVerdictCache? l0Cache = null)
    {
        ...
        CancellationToken ct = default)
    {
        ...
        // L0: in-process verdict cache, populated synchronously by
        // BotDetectionMiddleware right after the pipeline returns. Closes the
        // cold-start race that was making parallel HTTP/2 asset bursts from the
        // same visitor each pay the full pipeline cost. Bug O regression: same
        // primarySig, 5 requests within 1 second, first two missed both sigCoord
        // and identity-store (the SQLite row hadn't been written yet) and ran
        // duplicate pipelines.
        var l0SwStart = System.Diagnostics.Stopwatch.GetTimestamp();
        var l0Verdict = _l0Cache?.TryGet(signature);
        var l0LookupMs = ElapsedMs(l0SwStart);

        if (l0Verdict is not null)
        {
            var l0Age = (DateTime.UtcNow - l0Verdict.LastSeenUtc).TotalSeconds;
            var l0SkipEligible =
                l0Verdict.Confidence >= options.SkipMinConfidence
                && l0Age <= options.SkipMaxAgeSeconds;
            if (l0SkipEligible && !ShouldRefresh(signature, options.SkipSamplingRate))
            {
                _logger.LogInformation(
                    "VerdictGate sig={SigPrefix} -> Skip (L0) conf={Conf:F2} age={AgeSec:F1}s l0Ms={L0Ms:F1} prob={Prob:F2}",
                    sigPrefix, l0Verdict.Confidence, l0Age, l0LookupMs, l0Verdict.BotProbability);
                return new GateDecision(GateAction.Skip, l0Verdict);
            }
        }
