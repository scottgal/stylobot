...
///     the slot-aware name updaters on <see cref="IFingerprintStore"/>
///     (<see cref="IFingerprintStore.UpdateInducedNameAsync"/>,
///     <see cref="IFingerprintStore.UpdateLlmNameAsync"/>,
///     <see cref="IFingerprintStore.UpdateGivenNameAsync"/>) whenever a fingerprint's
///     resolved display name (<c>given ?? llm ?? induced</c>) transitions to a new
///     non-empty value. Read by the signature timeline view on demand (NOT cached
///     in the LFU dicts -- snapshots are cold storage).
/// </summary>
