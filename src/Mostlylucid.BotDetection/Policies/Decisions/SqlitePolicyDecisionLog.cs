...
public sealed class SqlitePolicyDecisionLog : IPolicyDecisionLog, IAsyncDisposable
{
    ...
    private void InitialiseSchema()
    {
        ...
        cmd.CommandText = """
            ...
            CREATE INDEX IF NOT EXISTS ix_policy_decisions_rule_observed   ON policy_decisions(rule_id, observed_at);
            CREATE INDEX IF NOT EXISTS ix_policy_decisions_winner_observed ON policy_decisions(winner_rule_id, observed_at);
            CREATE INDEX IF NOT EXISTS ix_policy_decisions_fingerprint     ON policy_decisions(fingerprint, observed_at);
            """;
        cmd.ExecuteNonQuery();
    }
    ...
    private void DrainBatch()
    {
        ...
        cmd.CommandText = """
            INSERT INTO policy_decisions
                (rule_id, winner_rule_id, matched, fingerprint, action_kind, mode, eval_ticks, observed_at)
            VALUES
                (@rule_id, @winner_rule_id, @matched, @fingerprint, @action_kind, @mode, @eval_ticks, @observed_at)
            """;
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            await using var cmd = _connection.CreateCommand();
            cmd.CommandText = """
                SELECT rule_id, winner_rule_id, matched, fingerprint, action_kind, mode, eval_ticks, observed_at
                FROM policy_decisions
