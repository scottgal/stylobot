...
- **Remaining**: the Haxxor YAML category lists (step 3), the `ResponseCoordinator` default (step 4), `EndpointRiskClassifier` (step 5 cont.), inline references in `SignatureToBdfMapper` / `HeuristicFeatureExtractor` / `ThreatIntelAtom` (step 6), and the `AdditionalCatalogFiles` extension surface (step 7).
...
| `src/Mostlylucid.BotDetection/Behavioral/SignatureToBdfMapper.cs`, `Detectors/HeuristicFeatureExtractor.cs`, `Orchestration/Atoms/ThreatIntelAtom.cs` | Inline references | Constants from the catalog |
...
6. Inline references in `SignatureToBdfMapper` / `HeuristicFeatureExtractor` / `ThreatIntelAtom` → constants from the catalog.
