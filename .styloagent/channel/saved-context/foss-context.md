...
- **BIG FINDING from the "sweep for other readers" ask**: the systemic sink-only-hint/key-mismatch bug class (3rd occurrence: cli-'s CLI TUI, my dashboard nav link, now this) also breaks the CANONICAL `SignatureLookup.cs` helper — confirmed dead in production for ALL its ~6 real call sites: `RateLimitKeyResolver.GetSignature` (per-signature rate-limit keying never engages), `ChallengeActionPolicy` (challenges always fall back to raw-IP binding, never the real signature), `BlockResponseGate` (Holodeck fingerprint tracking never set on honeypot/attack paths), `StyloBotEdgeHeaders` (has its own safer fallback), `YarpExtensions.cs` (gateway's X-Bot-Detection-PrimarySignature header build — UNCONFIRMED whether remote-mode forwarding is fully broken or something else compensates). This is detection/enforcement-affecting, not just UI — recommended NOT bundling a rushed fix into tonight's build; filed as its own high-severity tracked issue (`systemic-canonical-signaturelookup-helper-is-dea`) with a recommended root-level fix (populate `signals[SignalKeys.PrimarySignature]` in `DetectionLedgerExtensions.ToAggregatedEvidence` via the existing sink-first ReadString pattern, same shape as the RiskJustification injection). Escalated urgent to overview-, awaiting their read on prioritization/scope before touching it further.
- Worktrees in play: `stylobot-sig-fix` (foss-signature-extraction-fix), `stylobot-internal-filter` (#1 FOSS), `stylobot-commercial-internal-filter` (#1 commercial), `stylobot-window-threading` (merged to main, can be cleaned up).

## CURRENT STATE (2026-07-25, even later)
- Found + fixed a REAL bug in my own just-shipped "Your Signature" nav link, prompted by cli-'s unrelated FYI about the same defect class: `DetectionDataExtractor.ExtractSignatures` had two dead retrieval paths on any in-process (non-remote-gateway) host — (1) reads `AggregatedEvidence.Signals` for signature.multifactor/primary, which SignatureAtom only ever sink.Raise()s (never populates production Signals), (2) a key-name mismatch (`SignalKeys.SignatureMultifactor` = "signature.multifactor" vs `SignatureAtom.MultifactorKey` = "stylobot.signature.multifactor" — literally different strings). Net effect: every consumer (Your Detection panel, my nav link) silently fell back to a transient, never-persisted SHA256 IP+UA hash. Fixed by checking `SignatureAtom.TryGetMultifactor(context)` first. TDD: reproduced RED (fallback hash), fixed GREEN (real signature). Full suite 4627/4633, 0 failed. Branch `foss-signature-extraction-fix` (worktree `stylobot-sig-fix`, commit `4f983235`, off FOSS main 3e76fe8d). Flagged urgent to overview- — held pending review + direct operator push confirmation, same as everything else.
- overview- said HOLD #1 (both repos, foss-internal-filter-posture + commercial-internal-filter-posture) — bundle with mae-'s domain-filter build when operator green-lights both, likely morning. Filed the flaky-test issue overview- asked for (pre-existing-test-infra-flakiness).
...
- Endpoint list-vs-detail #2 (leading-slash hypothesis): overview-/deploy- proposed a leading-slash mismatch as the "not found" root cause. I RAN the exact current parsing code (`rest[slashIdx..]` keeps the leading slash) and it produces the CORRECT segPath ('/metrics', matching the DB) — empirically verified, not just re-read. Reported this counter-finding back; the code as currently written should NOT have this bug. Likely explanation: staging deployed build predates the route, or something not yet found. NOT implementing a fix until this is resolved — don't want to "fix" a correct code path and introduce a double-slash regression. Awaiting overview-/deploy-'s next data point (deployed SHA check or a debug log line).
