...
Cross-request bot network detection using label propagation, FFT-based spectral analysis, and time-decayed country reputation tracking.
...
SignatureCoordinator      BotClusterService (background)
    |                      /          \
    |          Label Propagation    Spectral Analysis (FFT)
    |                 |                    |
    v                 v                    v
...
|-----------|------|---------|
| `BotClusterService` | `BackgroundService` | Periodically clusters bot signatures using label propagation |
